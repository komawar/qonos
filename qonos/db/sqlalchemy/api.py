...
def paginate_query(query, model, limit=None, marker=None, sort_keys,
...
def schedule_get_all(filter_args={}):
    ...
    if filter_args['marker'] is not None:
        marker_schedule = _schedule_get_by_id(filter_args['marker'])

    query = paginate_query(query, models.Schedule, limit=filter_args['limit'],
                               marker=marker_schedule, ['id'])
